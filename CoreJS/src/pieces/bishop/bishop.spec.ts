...
import { BishopMoveGenerator, BlackBishopNotation, WhiteBishopNotation } from "./bishop";
...
describe("BishopMoveGenerator", () => {
  const makeBbs = (whiteBishops: Bitboard, blackBishops: Bitboard, whitePieces: Bitboard = 0n, blackPieces: Bitboard = 0n) =>
    new Map([
      [WhiteBishopNotation, whiteBishops],
      [BlackBishopNotation, blackBishops],
  ...
  it("generates diagonal moves for center bishop (e4)", () => {
    ...
    const expected = ["f5", "g6", "h7", "d5", "c6", "b7", "a8", "f3", "g2", "h1", "d3", "c2", "b1"].map(getBBfromSquare).reduce((a, b) => a | b, 0n as Bitboard);
  ...
  it("handles corner squares correctly (a1)", () => {
    ...
    const expected = ["b2", "c3", "d4", "e5", "f6", "g7", "h8"].map(getBBfromSquare).reduce((a, b) => a | b, 0n as Bitboard);
